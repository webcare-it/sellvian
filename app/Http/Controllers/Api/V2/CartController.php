...
class CartController extends Controller
{
    ...
    {
        $items = Cart::where('user_id', $user_id)->orWhere('temp_user_id', $user_id)->get();
    ...
    {
        $owner_ids = Cart::where('user_id', $user_id)->orWhere('temp_user_id', $user_id)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray();
        $currency_symbol = currency_symbol();
        $shops = [];
        if (!empty($owner_ids)) {
            foreach ($owner_ids as $owner_id) {
                $shop = array();
                $shop_items_raw_data = Cart::where('user_id', $user_id)->orWhere('temp_user_id', $user_id)->where('owner_id', $owner_id)->get()->toArray();
    ...
    public function updateShippingCostInCart(Request $request)
    {
        // Validate request
        $request->validate([
            'user_id' => 'required',
        ...
        $cartItems = Cart::where('user_id', $request->user_id)
            ->orWhere('temp_user_id', $request->user_id)
            ->get();

        if ($cartItems->isEmpty()) {
            return response()->json([
                'result' => false,
                'message' => translate('No cart items found for this user'),
            ]);
        }

        // Divide shipping cost equally among items
        $shipping_cost_per_item = $request->shipping_cost / $cartItems->count();

        // Update all cart rows
        Cart::where('user_id', $request->user_id)
            ->orWhere('temp_user_id', $request->user_id)
            ->update(['shipping_cost' => $shipping_cost_per_item]);

        return response()->json([
            'result' => true,
            'message' => translate('Shipping cost updated successfully'),
        ]);
    }
    ...
    public function add(Request $request)
    {
        ...
        Cart::updateOrCreate(
            [
                ...
                'variation'  => $variant,
                // Use either user_id or temp_user_id depending on request
                'user_id'    => $request->has('user_id') ? $request->user_id : null,
                'temp_user_id' => $request->has('temp_user_id') ? $request->temp_user_id : null,
            ],
            [
...
//        }
    ...
    public function changeQuantity(Request $request)
    {
        ...
        if ($cart != null) {
            // Check if product has variant or not
            if ($cart->variation != null) {
                // Variant product stock
                $product_stock = $cart->product->stocks->where('variant', $cart->variation)->first();
            } else {
                // Simple product stock (no variant)
                $product_stock = $cart->product->stocks->first();
            }

            // If stock record exists
    ...
    public function destroy($id)
    {
        ...
        // Get remaining cart items for the user (after removal)
